...
{
    public partial class FBusquedaCargo : Form
    {
        ...
        public FBusquedaCargo()
        {
            InitializeComponent();
            this.AcceptButton = button3; //Se asigna el botón Aceptar como botón por defecto
        }

        private void FBusquedaCargo_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
        ...
        private void FBusquedaCargo_FormClosing(object sender, FormClosingEventArgs e)
        {
            if(caso == false)
            {
        ...
        {
            DataTable CargoConsultar;

            // Si no se ingresó ningún parámetro, cargar todos los datos
            if (string.IsNullOrWhiteSpace(valorparametro))
            {
                CargoConsultar = objCargo.CargoTodo(); // Llama al procedimiento que trae todos los registros
            }
            else
            {
                CargoConsultar = objCargo.CargoConsultar(valorparametro); // Llama al procedimiento filtrado
            }

            if (CargoConsultar != null && CargoConsultar.Rows.Count > 0)
            {
                dataGridView1.DataSource = CargoConsultar;
