...
{
    public partial class FBusquedaFactura : Form
    {
        ...
        public FBusquedaFactura()
        {
            InitializeComponent();
            this.AcceptButton = button3; //Se asigna el botón Aceptar como botón por defecto
        }

        private void FBusquedaFactura_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.vidfactura = 0; //variable global que tomará el valor seleccionado
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    Program.vidfactura = Convert.ToInt32(dataGridView1.CurrentRow.Cells[0].Value);
                    Program.parametro = Program.vidfactura.ToString(); //Se asigna el valor a la variable global

                    // Aquí puedes agregar el código para cargar los datos basados en el ID
                    MessageBox.Show("Factura seleccionada: " + Program.parametro, "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    // Si este formulario es un diálogo, puedes cerrarlo con DialogResult.OK
                    this.DialogResult = DialogResult.OK;

                    // Cambiamos el valor de caso para que no mandarlo al menu principal
                    caso = true;
                    Program.controlmov = 3;
                ...
                {
                    MessageBox.Show("Por favor, seleccione una Factura de la lista", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FBusqueraFactura_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (caso == false)
            {
        ...
        {
            DataTable FacturaConsultar;

            // Si no se ingresó ningún parámetro, cargar todos los datos
            if (string.IsNullOrWhiteSpace(valorparametro))
            {
                FacturaConsultar = objFactura.FacturaTodo(); // Llama al procedimiento que trae todos los registros
            }
            else
            {
                FacturaConsultar = objFactura.FacturaConsultar(valorparametro); // Llama al procedimiento filtrado
            }

            if (FacturaConsultar != null && FacturaConsultar.Rows.Count > 0)
            {
                dataGridView1.DataSource = FacturaConsultar;

                // Configurar el ancho de las columnas según la información de facturas
