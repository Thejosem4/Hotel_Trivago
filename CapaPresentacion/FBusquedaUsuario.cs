...
namespace CapaPresentacion
{
    public partial class FBusquedaUsuario : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    Program.vidusuario = Convert.ToInt32(dataGridView1.CurrentRow.Cells[0].Value);
                    Program.parametro = Program.vidusuario.ToString(); //Se asigna el valor a la variable global

                    // Aquí puedes agregar el código para cargar los datos basados en el ID
                    MessageBox.Show("Usuario seleccionado: " + Program.parametro, "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            DataTable UsuarioConsultar;

            // Si no se ingresó ningún parámetro, cargar todos los datos
            if (string.IsNullOrWhiteSpace(valorparametro))
            {
                UsuarioConsultar = cNUsuario.UsuarioTodo(); // Llama al procedimiento que trae todos los registros
            }
            else
            {
                UsuarioConsultar = cNUsuario.UsuarioConsultar(valorparametro); // Llama al procedimiento filtrado
            }

            if (UsuarioConsultar != null && UsuarioConsultar.Rows.Count > 0)
            {
                dataGridView1.DataSource = UsuarioConsultar;
