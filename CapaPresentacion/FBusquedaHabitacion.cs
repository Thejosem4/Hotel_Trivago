...
{
    public partial class FBusquedaHabitacion : Form
    {
        public int indice = 0, vtieneparametro = 0;
        public string valorparametro = "";
        private bool caso;
        private CNHabitacion cNHabitacion = new CNHabitacion();

        public FBusquedaHabitacion()
        {
            InitializeComponent();
            this.AcceptButton = button3; //Se asigna el botón Aceptar como botón por defecto
        }

        private void FBusquedaHabitacion_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.vidhabitacion = 0; //variable global que tomará el valor seleccionado
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    Program.vidhabitacion = Convert.ToInt32(dataGridView1.CurrentRow.Cells[0].Value);
                    Program.parametro = Program.vidhabitacion.ToString(); //Se asigna el valor a la variable global

                    // Aquí puedes agregar el código para cargar los datos basados en el ID
                    MessageBox.Show("Empresa seleccionado: " + Program.parametro, "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void FBusquedaHabitacion_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (caso == false)
            {
        ...
        {
            DataTable HabitacionConsultar;

            // Si no se ingresó ningún parámetro, cargar todos los datos
            if (string.IsNullOrWhiteSpace(valorparametro))
            {
                HabitacionConsultar = cNHabitacion.HabitacionTodo(); // Llama al procedimiento que trae todos los registros
            }
            else
            {
                HabitacionConsultar = cNHabitacion.HabitacionConsultar(valorparametro); // Llama al procedimiento filtrado
            }

            if (HabitacionConsultar != null && HabitacionConsultar.Rows.Count > 0)
            {
                dataGridView1.DataSource = HabitacionConsultar;
