...
{
    public partial class FBusquedaDepartamento : Form
    {
        ...
        //Evento Load
        private void FBusquedaDepartamento_Load(object sender, EventArgs e)
        {
            valorparametro = "";
            vtieneparametro = 0;
            Program.viddepartamento = 0; //variable global que tomará el valor seleccionado
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    Program.viddepartamento = Convert.ToInt32(dataGridView1.CurrentRow.Cells[0].Value);
                    Program.parametro = Program.viddepartamento.ToString(); //Se asigna el valor a la variable global

                    // Aquí puedes agregar el código para cargar los datos basados en el ID
                    MessageBox.Show("Departamento seleccionado: " + Program.parametro, "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    MessageBox.Show("Por favor, seleccione un departamento de la lista", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al seleccionar el departamento: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void FBusquedaDepartamento_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (caso == false)
            {
        ...
        {
            DataTable DepartamentoConsultar;

            // Si no se ingresó ningún parámetro, cargar todos los datos
            if (string.IsNullOrWhiteSpace(valorparametro))
            {
                DepartamentoConsultar = objDepartamento.DepartamentoTodo(); // Llama al procedimiento que trae todos los registros
            }
            else
            {
                DepartamentoConsultar = objDepartamento.DepartamentoConsultar(valorparametro); // Llama al procedimiento filtrado
            }

            if (DepartamentoConsultar != null && DepartamentoConsultar.Rows.Count > 0)
            {
                // Asigna el DataSource del DataGridView a los datos del método DepartamentoConsultar
                dataGridView1.DataSource = DepartamentoConsultar; // Se ejecuta el método para mostrar los datos
                dataGridView1.Columns[0].Width = 100; // id_departamento
